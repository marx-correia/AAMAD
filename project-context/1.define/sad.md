...
- **ADR-013 Sprint 1 = single vertical slice, not product-complete MVP (2026-08-30 sharpening)**. Rationale: the prior SAD read as a product-complete MVP (OAuth SSO polish, managed Postgres + vector store, live CRM + ticketing + email, five agents wired to real APIs, full F6 suite, container deploy at 100 concurrent sessions). The Build phase risks swallowing weeks before a single grounded onboarding turn ships. Sprint 1 is therefore explicitly re-scoped to **one vertical path** (see "Sprint 1 Scope (Vertical Slice)" and "Deferred — Post-Sprint-1" subsections below). Consequences: (a) all external integrations (CRM, KB, ticketing, email) MUST have fixture implementations behind the production interface for Sprint 1 (ADR-014); (b) SSO polish, ADR-008 `human_input` sponsor-brief gate, ADR-011 full ambiguity calibration (composite signal tuning, N=2 telemetry), and live third-party APIs stay SPECIFIED but DEFERRED — none of them are Sprint 1 blockers; (c) the Sprint 1 exit criterion is a working vertical, not a feature-complete crew; (d) OQ-1 (LLM default) and OQ-2 (session cost ceiling) are pinned as Build assumptions so `max_iter`/`max_tokens` are not free variables (see "Sprint 1 Pinned Build Assumptions" below); (e) the ≤500ms first token / ≤3s p95 turn targets are treated as a spike gate measured in Week 3, not as a fix-with-more-agents problem.
- **ADR-014 Stub-first tool interfaces (2026-08-30 sharpening)**. Rationale: Integration blocks on vendor accounts while the crew contract is still unproven; stubbing every external tool on day one lets `@backend.eng` prove the crew end-to-end without waiting for CRM/KB/ticket/email credentials. Decision: `crm_lookup`, `kb_vector_search`, `ticket_api`, and the email delivery hook MUST ship with fixture implementations behind the SAME interface as production; production adapters are behind a factory switched by env var (`CRM_MODE=stub|live`, `KB_MODE=stub|live`, `TICKET_MODE=stub|live`, `EMAIL_MODE=stub|live`). Seed KB fixture MUST include 3–5 chunks split into **Path A** (grounded-answer hit) and **Path B** (no hit → clarify/escalate). See §2 "Fixture-Tool Interface Contracts (ADR-014)" for the interface contracts and fixture package location. Consequence: `@integration.eng` is unblocked on Day 1 and can flip modes to `live` per tool, per environment, without touching agent/task code.
- **ADR-015 Pydantic-typed task outputs via `output_pydantic` (2026-08-30 sharpening)**. Rationale: `expected_output` prose can drift silently between the SAD and running code. Bind each task's output to a Pydantic v2 model via CrewAI's `output_pydantic` so schema drift fails fast at Build time. Decision: the following models are the CONTRACT (see §2 "Pydantic Output Contracts (ADR-015)" for full schemas): `CustomerProfile` (welcome_task), `OnboardingPlan` (plan_task), `GroundedSupportAnswer` (first_use_support_task), `EscalationTicketPayload` (escalation_task), `AmbiguityReport` (embedded field, produced by `context_packager` on ambiguity escalations), `SponsorBrief` (sponsor_brief_task; kept prose-body but framed by a typed metadata envelope). These models supersede the JSON snippets already in §2 as the runtime contract; the JSON snippets remain as informative documentation. Consequence: `@backend.eng` MUST declare `output_pydantic=<Model>` on the CrewAI `Task` definition; a `Task.guardrail` schema-check is redundant with `output_pydantic` for shape but is still required for content rules (citation presence, N=2 cap, "standard defaults" label).
- **ADR-016 Orchestrator pattern in `crew.py` (branching, not linear kickoff) (2026-08-30 sharpening)**. Rationale: PRD flows are not linear — Welcome routes to sponsor OR plan; first-use support may loop then escalate on 2 failed resolution attempts OR explicit "Talk to a human". A single `crew.kickoff()` call over a fixed sequential task list cannot express that. Decision: `crew.py` implements an **explicit per-turn orchestrator** (see §2 "Orchestrator Pattern in `crew.py` (ADR-016)") that (a) determines the current phase from session state, (b) invokes exactly the task(s) needed for that turn, (c) propagates `CustomerProfile` (and `ambiguity_flags[]`) via `Task.context` between agents, (d) triggers `escalation_task` on 2 failed resolution attempts (tracked on `SessionState.support_failure_count`) OR explicit user request — WITHOUT relying on unbounded `allow_delegation`. `allow_delegation=false` is now the SAD-preferred default for ALL agents in Sprint 1 (tightens ADR-006 for Sprint 1 only; ADR-006 remains authoritative post-Sprint-1). Consequence: the phase router in `crew.py` is the single source of truth for "which task runs this turn"; `Task.context` is the single source of truth for "what data the next task sees"; guardrails and Pydantic models enforce the contract at every hop.
...
**Sprint 1 Scope (Vertical Slice) (binds ADR-013 / ADR-014 / ADR-015 / ADR-016)**

Sprint 1 ships one working end-to-end path. Everything else is DEFERRED (see the following subsection). Build personas MUST NOT re-expand scope without an explicit SAD update.

- **Sprint 1 exit criteria (verbatim, authoritative):**
    > anonymous/dev session → welcome qualify (stub CRM) → plan JSON in chat → one grounded support turn with citation (fixture KB) → escalate with context pack (stub ticket).
- **Concrete Week-3 target (the Build slice to ship):**
    - Chat composer sends a message → `POST /v1/sessions/{id}/messages` opens an SSE stream (per §4 SSE envelope — this contract is NOT weakened).
    - `welcome_task` runs against a stub CRM, produces a `CustomerProfile` (Pydantic — see ADR-015).
    - `plan_task` receives `CustomerProfile` via `Task.context` and renders an `OnboardingPlan` bubble in chat.
    - One support turn against fixture KB (Path A) returns a `GroundedSupportAnswer` with a `cite` SSE event.
    - Explicit "Talk to a human" (or 2 failed resolution attempts, tracked as `SessionState.support_failure_count`) triggers `escalation_task` → returns a stub ticket id via `state=escalate` SSE event.
    - `CustomerProfile` is preserved across the turn via `Task.context` (no re-asking).
    - Invariant: the crew MUST never emit a customer-specific support answer without a citation OR cleanly escalate — refuse-and-escalate is the deterministic fallback (aligns with ADR-009).
- **Sprint 1 spike gate (Week 3 measurement):**
    - Measure one welcome turn and one support turn against the fixture KB. If first-token p95 > 500ms OR full-turn p95 > 3s: keep SSE for UX, but do NOT add agents or memory to "fix" latency. Fixes stay within model, prompt size, and rolling-context-window levers (§7). This gate is a decision point, not a rollback trigger.
- **Sprint 1 is EXPLICITLY out of scope for:** OAuth SSO polish, ADR-008 sponsor-brief `human_input` gate, ADR-011 ambiguity signal calibration and N=2 telemetry, real third-party CRM/KB/ticketing/email APIs, `@security.eng` full assessment, 100 concurrent sessions load target, container hardening, multi-tenant SSO isolation smoke, F6 full guardrail suite in CI, per-agent memory. All of these are DEFERRED — see the next subsection.

**Deferred — Post-Sprint-1 (SPECIFIED but not built in Sprint 1)**

The following architectural commitments remain SPECIFIED in this SAD (no rework needed) but are DEFERRED — they are NOT Sprint 1 blockers, and Build personas MUST NOT pull them into Sprint 1 without an ADR update.

- **OAuth SSO polish and PKCE flow** — dev sessions are anonymous or dev-token-based in Sprint 1; the OAuth2 flow specified in §4/§8 stays deferred until the vertical slice is green. (Rationale: unblocks Build without waiting for OAuth issuer wiring.)
- **ADR-008 sponsor-brief `human_input=true` review gate** — the `sponsor_brief_agent` and `sponsor_brief_task` stay specified in §2; not exercised in Sprint 1. The customer-visible content risk is deferred with sponsor brief itself.
- **ADR-011 full ambiguity calibration** — the ambiguity contract, `ambiguity_flags[]`, `AmbiguityReport`, `Task.guardrail` bindings, `ambiguity-*.jsonl` stream, and Ambiguity Observability Hook all stay in the SAD unchanged. Sprint 1 exercises ONLY the escalation path (2 failed resolutions OR explicit user request); composite signal calibration, N=2 telemetry, KPI dashboards, and the full guardrail test suite (§9) are DEFERRED.
- **Live third-party APIs** — CRM, KB vector search, ticketing, and email use fixture implementations per ADR-014 in Sprint 1; live adapters ship post-Sprint-1 by flipping the mode env var per environment.
- **Full F6 test suite in CI** — the F6 guardrail suite in §9 remains specified; Sprint 1 requires only the "refuse-and-escalate on missing citation" test to be green (aligns with ADR-009).
- **100 concurrent sessions load target and container hardening** — §7 targets remain authoritative for post-Sprint-1; Sprint 1 validates single-user vertical only.
- **Per-agent memory** — remains OFF (ADR-005) and stays deferred; do not enable to "fix" latency (spike gate rules).

**Sprint 1 Pinned Build Assumptions (OQ-1 and OQ-2 pinned)**

To prevent `max_iter`/`max_tokens` from being free variables during Build, the following Open Questions are PINNED as Sprint 1 Build assumptions. These do NOT resolve the underlying Open Questions (which remain in "Open Questions" for stakeholder action) — they merely pin defaults so Build is not blocked.

- **PIN-OQ-1 (LLM default)** — Sprint 1 assumes Anthropic Claude Sonnet-class (per ADR-007), temperature ≤ 0.3, `max_tokens` budget per task from §7. Any deviation MUST be recorded in Audit and MUST re-validate the Week-3 spike gate. Underlying stakeholder decision (final provider/tier) remains in Open Questions (OQ-1 propagated from PRD).
- **PIN-OQ-2 (session cost ceiling)** — Sprint 1 assumes a working default of **US$ 0.25 per completed onboarding session** (welcome + plan + 1 support turn + optional escalate) for the Week-3 spike gate. This is a Build placeholder, NOT a stakeholder-approved contract. `max_iter` per agent (§2) and rolling-context-window default = 8 turns (§7) are the primary levers to hold this budget. Underlying stakeholder decision remains in Open Questions (OQ-2 propagated from PRD). If measured cost > US$ 0.25/session in Week 3, halt and report per adapter-crewai Failure Policy — do NOT silently increase `max_iter` or add agents.
- **Spike gate coupling** — the ≤500ms first-token and ≤3s p95 turn NFRs from §7 are exercised in Week 3 against these pinned assumptions. If PIN-OQ-1 or PIN-OQ-2 changes, the spike gate MUST be re-run.
...
**Pydantic Output Contracts (ADR-015)**

Each task's output is bound to a Pydantic v2 model via CrewAI `Task(..., output_pydantic=<Model>)`. The models below are the RUNTIME CONTRACT and supersede the informative JSON snippets earlier in §2 for shape enforcement. Content rules (citation presence, `standard defaults` labeling, N=2 cap) continue to be enforced by `Task.guardrail` — `output_pydantic` handles shape, guardrail handles semantics.

- **`CustomerProfile`** — bound to `welcome_task`. Fields: `session_id: str`, `tenant_id: str`, `role: Literal["implementer","sponsor","unknown"]`, `primary_use_case: str | None`, `timeline: str | None`, `locale: Literal["en","pt-BR"]`, `ambiguity_flags: list[AmbiguityFlag] = []`, `confidence: float`, `next_track: Literal["plan","sponsor_brief","clarify","escalate"]`, `created_at: datetime`. `AmbiguityFlag` schema is the JSON schema already documented above under "Ambiguity Contracts" (`session_id`, `instance_id`, `ambiguity_type`, `affected_slot_or_intent`, `confidence`, `clarification_attempts`, `resolved`, `fallback_applied`, `template_default_id`, `detected_by_agent`, `created_at`).
- **`OnboardingPlan`** — bound to `plan_task`. Fields: `session_id: str`, `milestones: list[Milestone]` (3–5 items per PRD F2/AC2), `generated_from_profile_hash: str` (hash of `CustomerProfile` used as input — traceability), `ambiguity_flags: list[AmbiguityFlag] = []`, `created_at: datetime`. `Milestone`: `id: str`, `title: str`, `estimated_hours: float` (≤ 2h per PRD F2/AC2), `doc_url: str`, `template_id: str | None`, `order: int`.
- **`GroundedSupportAnswer`** — bound to `first_use_support_task`. Fields: `session_id: str`, `answer_markdown: str`, `citations: list[Citation]` (MUST be non-empty when `refused=False` — enforced by ADR-009 guardrail), `retrieval_top_score: float`, `retrieval_delta: float`, `confidence: float`, `refused: bool`, `refusal_reason: str | None`, `ambiguity_flags: list[AmbiguityFlag] = []`. `Citation`: `source_id: str`, `url: str`, `score: float`, `chunk_id: str`. Invariant: `refused=True` XOR `len(citations) > 0`.
- **`EscalationTicketPayload`** — bound to `escalation_task`. Fields: `session_id: str`, `tenant_id: str`, `ticket_id: str`, `customer_profile: CustomerProfile`, `transcript_window: list[TranscriptTurn]`, `what_was_tried: list[str]`, `trigger: Literal["two_failed_resolutions","user_requested","ambiguity","guardrail_failure"]`, `ambiguity_report: AmbiguityReport | None`, `escalation_target: Literal["human_queue","specialist_queue"] = "human_queue"`, `created_at: datetime`.
- **`AmbiguityReport`** — embedded on `EscalationTicketPayload.ambiguity_report` when `trigger == "ambiguity"`. Shape mirrors the JSON schema already documented under "Ambiguity Contracts" (`session_id`, `instance_id`, `ambiguity_type`, `affected_slot_or_intent`, `confidence`, `missing_signals`, `clarification_attempts`, `fallback_applied`, `template_default_id`, `escalation_target`, `next_agent`, `trace_ref`) — promoted to a typed model.
- **`SponsorBrief`** — bound to `sponsor_brief_task` (DEFERRED per Sprint 1 scope; kept here for post-Sprint-1). Fields: `session_id: str`, `body_markdown: str` (≤ 1 page per PRD F5/AC1), `expected_time_to_value_days: int`, `implementer_plan_status: Literal["not_started","in_progress","complete","unknown"]`, `defaults_applied: list[str]` (labels the "based on standard defaults — pending confirmation" per ADR-011 F6/AC3), `human_review_required: bool` (defaults to `SPONSOR_BRIEF_REQUIRE_REVIEW` env — ADR-008), `ambiguity_flags: list[AmbiguityFlag] = []`.

Binding rule: `@backend.eng` MUST declare `output_pydantic=<Model>` on the corresponding `Task` in `crew.py` (models cannot be declared in `config/tasks.yaml` YAML alone — the class binding lives in `crew.py`). Schema drift between the SAD and running code MUST fail Build (Pydantic validation error on kickoff or unit test).

**Orchestrator Pattern in `crew.py` (ADR-016)**

Sprint 1 orchestration is NOT a single linear `crew.kickoff()` over a fixed task list. It is an explicit per-turn phase router. `allow_delegation=false` on ALL agents in Sprint 1 (tightens ADR-006; ADR-006 remains authoritative post-Sprint-1). Delegation-like behavior is expressed by the orchestrator choosing which task to run, NOT by an agent invoking another agent.

- **Session state (owned by orchestrator, persisted to Postgres):** `SessionState` = { `session_id`, `phase: Literal["welcome","plan","sponsor_brief","support","escalate","done"]`, `customer_profile: CustomerProfile | None`, `support_failure_count: int = 0`, `pending_ambiguity_instance_id: str | None`, `transcript_window: list[TranscriptTurn]` }.
- **Phase router (per turn, executed on every `POST /v1/sessions/{id}/messages`):**
    1. Load `SessionState` from Postgres (fresh state for a new `session_id`; `phase="welcome"` on first turn).
    2. If `phase == "welcome"`: run `welcome_task` → `CustomerProfile`. If `CustomerProfile.next_track == "plan"` → set `phase="plan"`. If `next_track == "sponsor_brief"` → set `phase="sponsor_brief"` (Sprint 1: DEFERRED — orchestrator raises `NotImplementedError` and falls through to `phase="plan"` for the vertical slice). If `next_track == "clarify"` → stay in `phase="welcome"` and emit clarification question (subject to N=2 cap on the `AmbiguityFlag` — ADR-011). If `next_track == "escalate"` → set `phase="escalate"`.
    3. If `phase == "plan"`: run `plan_task` with `Task.context=[welcome_task]` (or a hydrated `CustomerProfile` fixture in Sprint 1) → `OnboardingPlan`. Then set `phase="support"` (chat is now open for grounded Q&A).
    4. If `phase == "support"`: run `first_use_support_task` with `Task.context=[welcome_task, plan_task]` → `GroundedSupportAnswer`. If `refused=True`, increment `support_failure_count`. If `support_failure_count >= 2` OR the user message matches the "talk to a human" affordance regex/intent, set `phase="escalate"`.
    5. If `phase == "escalate"`: run `escalation_task` with `Task.context=[welcome_task, plan_task, first_use_support_task]` → `EscalationTicketPayload`. Emit `event: state` `data: { "phase": "escalate", "ticketId": ... }` on SSE. Set `phase="done"`.
- **`Task.context` propagation:** the orchestrator wires each downstream `Task` with `context=[<upstream_task_ids>]` per the phase router. `CustomerProfile` is the canonical carrier; `ambiguity_flags[]` on it is monotonic across agents (ADR-011). No agent may re-detect ambiguity on a slot already present in `ambiguity_flags[]`.
- **Escalation triggers (explicit, not delegated):**
    - `support_failure_count >= 2` (PRD F4/AC1)
    - Explicit user request ("Talk to a human" affordance click, or NL intent match)
    - `Task.guardrail` failure on `first_use_support_task` (missing citation — ADR-009)
    - Ambiguity fallback path (ii) per ADR-011 (post-Sprint-1)
- **What is NOT used:** `allow_delegation=true`, hierarchical process mode, `kickoff_for_each` (sessions are not batchable), manager agents.
- **Determinism:** the phase router is a pure function of `SessionState` + incoming message. Same input → same task chain. This is verifiable by a unit test in `@qa.eng`'s test plan (§9).
- **Post-Sprint-1 (informational):** `allow_delegation=true` on `welcome_agent` and `first_use_support_agent` per ADR-006 remains the target end state; the tighter Sprint 1 default exists to keep the orchestrator explicit while the crew contract is proven.

**Fixture-Tool Interface Contracts (ADR-014)**

Every external tool ships with a fixture implementation behind the SAME interface as production. Mode is selected per tool per environment via env var; agents/tasks are unchanged when flipping between fixture and live.

- **Fixture package location:** `backend/tools/fixtures/` (Python package) — mirrors the production adapter package layout under `backend/tools/adapters/`. Both are consumed by `backend/tools/factory.py`, which selects the adapter based on env var (`{TOOL}_MODE=stub|live`).
- **`crm_lookup` interface:** `lookup(tenant_id: str, customer_hint: str) -> CRMRecord` where `CRMRecord = { customer_id: str, tenant_id: str, subscription_tier: Literal["free","pro","enterprise"], signup_date: date, primary_contact_role_hint: str | None }`. **Fixture behavior:** returns a deterministic `CRMRecord` keyed by `customer_hint` from a JSON seed file `backend/tools/fixtures/crm_seed.json`. Env: `CRM_MODE=stub` in Sprint 1.
- **`kb_vector_search` interface:** `search(query: str, tenant_id: str, top_k: int = 5) -> list[KBHit]` where `KBHit = { chunk_id: str, source_id: str, url: str, text: str, score: float }`. **Fixture behavior:** returns hits from a fixture KB of 3–5 chunks in `backend/tools/fixtures/kb_seed.jsonl`. Env: `KB_MODE=stub` in Sprint 1.
    - **Path A (grounded-answer hit):** at least 2 chunks whose `text` contains a canonical answer for a well-known Sprint 1 test query (e.g., "how do I reset my API key" → deterministic hit with `score >= 0.75` and `delta >= 0.15` between top-1 and top-2 — clears the ADR-011 grounding threshold).
    - **Path B (no hit → clarify/escalate):** at least 1 chunk whose top `score` is below the grounding threshold OR whose `delta` is `<= 0.1` for the Sprint 1 test query "how do I connect my custom OAuth provider" — forces the refuse-and-escalate deterministic fallback per ADR-009 and the ambiguity type (c) path per ADR-011.
    - Total fixture size: 3–5 chunks minimum (Sprint 1 seed); more may be added without SAD update.
- **`ticket_api` interface:** `create_ticket(payload: EscalationTicketPayload) -> TicketCreated` where `TicketCreated = { ticket_id: str, url: str, created_at: datetime }`. **Fixture behavior:** returns a synthetic `ticket_id` (e.g., `STUB-TKT-<uuid4>`), writes the payload as JSON under `project-context/2.build/logs/stub-tickets/<ticket_id>.json` so `@qa.eng` can assert payload completeness end-to-end. Env: `TICKET_MODE=stub` in Sprint 1.
- **Email delivery hook (sponsor brief link)** — DEFERRED for Sprint 1 (sponsor brief itself is deferred); interface documented for post-Sprint-1: `send(to: str, subject: str, body_markdown: str, link_url: str) -> EmailSent`. Env: `EMAIL_MODE=stub|live`.
- **Contract testing:** `@qa.eng` MUST include contract tests that run the same unit-test suite against BOTH the fixture and the live adapter (when live adapters exist). Sprint 1 runs the suite against fixtures only.
- **Live-mode gate:** flipping any tool to `live` in a non-dev environment MUST re-run the Week-3 spike gate and MUST re-check the security assessment for that integration (§8).
...
| Sprint 1 sharpening (2026-08-30 operator review) | Vertical slice: anonymous session → welcome (stub CRM) → plan JSON → 1 grounded support turn (fixture KB) → escalate (stub ticket) | §1 "Sprint 1 Scope (Vertical Slice)", §1 "Deferred — Post-Sprint-1", §1 "Sprint 1 Pinned Build Assumptions" | ADR-013 |
| Sprint 1 tool stubs | Fixture-first CRM / KB / ticket / email behind production interface | §2 "Fixture-Tool Interface Contracts", §4 Auth/Secrets (`{TOOL}_MODE` env vars) | ADR-014 |
| Sprint 1 typed task outputs | `output_pydantic` on each task; `CustomerProfile`, `OnboardingPlan`, `GroundedSupportAnswer`, `EscalationTicketPayload`, `AmbiguityReport`, `SponsorBrief` | §2 "Pydantic Output Contracts", §9 Testing (schema-drift unit test) | ADR-015 |
| Sprint 1 orchestrator | Phase router in `crew.py`; `Task.context` propagation of `CustomerProfile`; explicit escalation triggers (2 failed resolutions OR "Talk to a human"); `allow_delegation=false` for Sprint 1 | §2 "Orchestrator Pattern in `crew.py`" | ADR-016 |
| Sprint 1 spike gate | ≤500ms first-token / ≤3s p95 turn measured in Week 3 on pinned PIN-OQ-1 (LLM) + PIN-OQ-2 (session cost) — do NOT add agents/memory to "fix" latency | §1 "Sprint 1 Scope (Vertical Slice)" spike gate, §7 targets | ADR-013 |

## Implementation Guidance for AI Development Agents

**Sprint 1 (Vertical Slice) — READ §1 "Sprint 1 Scope (Vertical Slice)" and "Deferred — Post-Sprint-1" BEFORE STARTING. Do NOT pull deferred items into Sprint 1 without an ADR update.**

1. `@project.mgr` → `setup.md`: scaffold Python 3.11 backend (`config/agents.yaml`, `config/tasks.yaml`, `crew.py`, `pyproject.toml`, `backend/tools/fixtures/`, `backend/tools/adapters/`, `backend/tools/factory.py` per ADR-014), Next.js 14 frontend (App Router, TS strict, Tailwind), `.env.example` (including `CRM_MODE`, `KB_MODE`, `TICKET_MODE`, `EMAIL_MODE`), docker-compose for dev. Sprint 1: anonymous/dev session — SSO flow scaffolded but NOT wired.
2. `@frontend.eng` → `frontend.md`: implement chat composer + `<MessageList>` + `<Citation>` + `<TalkToHumanButton>` + SSE consumer (per §4 envelope) + i18n scaffolding (EN default; PT-BR post-Sprint-1). Sponsor brief view DEFERRED. **No backend wiring**.
3. `@backend.eng` → `backend.md`: implement CrewAI crew per YAML, FastAPI endpoints per §4, SSE envelope, `output_pydantic=<Model>` binding on each Task per ADR-015, phase router in `crew.py` per ADR-016, fixture tool adapters per ADR-014, `Task.guardrail` for citation-required on `first_use_support_task` (ADR-009). Full F6 ambiguity guardrail suite DEFERRED (only refuse-and-escalate on missing citation is required for Sprint 1).
4. `@integration.eng` → `integration.md`: wire FE ↔ BE via typed OpenAPI client generated from Pydantic v2 models (ADR-015 contracts); load fixture adapters for CRM/KB/ticket in Sprint 1; validate tool bindings on `/readyz` including fixture data seed check (3–5 KB chunks present, Path A + Path B). Live adapters DEFERRED.
5. `@qa.eng` → `qa.md`: unit tests for Pydantic contract adherence (schema drift fails Build); phase router determinism test (same `SessionState` + message → same task chain); Sprint 1 vertical-slice smoke (welcome → plan → grounded support turn → escalate); citation-required guardrail assertion. Full F6 test suite DEFERRED. Week-3 spike gate measurement (≤500ms first-token / ≤3s p95 turn) is a Sprint 1 deliverable.
6. `@security.eng` → `security.md`: DEFERRED for Sprint 1 vertical slice (fixture-only integrations, anonymous dev session). Full pre-Deliver security assessment (SSO flow, PII redaction, secret handling, tenant isolation) required before any tool flips to `live` in a non-dev environment.
7. `@devops.eng` → `deploy.md`: CI (lint/test/build) with fixture-mode default; single-region managed-container deploy config generated but NOT executed for Sprint 1. Rollback runbook stays specified. Do not trigger live deploys without operator authorization.
...
- **Sprint 1 scope tightened to a single vertical slice (ADR-013, 2026-08-30 sharpening).** The Sprint 1 exit criterion is: `anonymous/dev session → welcome qualify (stub CRM) → plan JSON in chat → one grounded support turn with citation (fixture KB) → escalate with context pack (stub ticket)`. All prior "product-complete MVP" commitments (OAuth SSO polish, live third-party APIs, ADR-008 human-input gate, ADR-011 ambiguity signal calibration, 100 concurrent sessions load target, full F6 guardrail suite in CI, per-agent memory) remain SPECIFIED but DEFERRED — see §1 "Deferred — Post-Sprint-1".
- **Sprint 1 tools are fixture-first (ADR-014).** `crm_lookup`, `kb_vector_search`, `ticket_api`, and the email hook use fixture implementations behind the same interface as production. Env vars `CRM_MODE`, `KB_MODE`, `TICKET_MODE`, `EMAIL_MODE` gate live adapters; Sprint 1 runs all in `stub` mode. Fixture KB seeded with 3–5 chunks covering Path A (grounded hit) and Path B (no hit → clarify/escalate).
- **Sprint 1 task outputs are Pydantic-typed (ADR-015).** `output_pydantic` bound to each task; models are the runtime contract (`CustomerProfile`, `OnboardingPlan`, `GroundedSupportAnswer`, `EscalationTicketPayload`, `AmbiguityReport`, `SponsorBrief`). Schema drift MUST fail Build. The JSON snippets earlier in §2 are informative documentation; the Pydantic models are authoritative.
- **Sprint 1 orchestrator is an explicit phase router (ADR-016).** `crew.py` implements a per-turn phase router; `allow_delegation=false` on ALL agents in Sprint 1 (tightens ADR-006 for Sprint 1 only); `CustomerProfile` propagated via `Task.context`; escalation triggered on `support_failure_count >= 2` OR explicit "Talk to a human" OR guardrail failure — NOT via unbounded delegation.
- **PIN-OQ-1 (LLM default) pinned for Sprint 1.** Anthropic Claude Sonnet-class per ADR-007, temperature ≤ 0.3, `max_tokens` per §7. Underlying stakeholder decision (final provider/tier) remains in Open Questions (item 1) — pin is a Build placeholder, not a stakeholder-approved contract.
- **PIN-OQ-2 (session cost ceiling) pinned for Sprint 1.** Working default: **US$ 0.25 per completed onboarding session** (welcome + plan + 1 support turn + optional escalate) for Week-3 spike gate. Primary levers: `max_iter` (§2) and rolling-context-window default = 8 turns (§7). Underlying stakeholder decision remains in Open Questions (item 2) — pin is a Build placeholder, not a stakeholder-approved contract. If measured cost > US$ 0.25/session at Week 3, halt and report per adapter-crewai Failure Policy; do NOT silently raise `max_iter`.
- **Week-3 spike gate is a decision point, not a rollback trigger.** If ≤500ms first-token OR ≤3s p95 turn misses under PIN-OQ-1 + PIN-OQ-2, remediation is bounded to model / prompt-size / context-window levers (§7). Adding agents or enabling memory is explicitly DISALLOWED as a latency remediation lever in Sprint 1.
...
18. **(2026-08-30, sharpening)** Is US$ 0.25 per completed session the right working ceiling for PIN-OQ-2, or does stakeholder want a tighter value? A tighter value tightens `max_iter` and rolling-context-window before Build starts. (→ stakeholder; unblocks final Build tuning)
19. **(2026-08-30, sharpening)** Where does the "Talk to a human" affordance intent match live — regex over user message, a dedicated affordance button on `<TalkToHumanButton>` only, or both? ADR-016 currently accepts either; final UX policy unclear. (→ UX / `@frontend.eng`)
20. **(2026-08-30, sharpening)** Should Sprint 1 include the phase-router unit test in CI, or is a manual Week-3 smoke acceptable? Current SAD posture (§9 Implementation Guidance): phase-router determinism test is a Sprint 1 deliverable. Confirm with `@qa.eng`. (→ `@qa.eng`)
21. **(2026-08-30, sharpening)** Fixture KB seed content ownership — does `@backend.eng` author the 3–5 chunks, or does Customer Success provide them? Content must include Path A (grounded hit) and Path B (no-hit → clarify/escalate) queries. (→ stakeholder / Customer Success)
...
- **Persona id:** `@system.arch`
- **Action:** `*review-and-adjust-sad-sprint-1-sharpening`
- **Resolved `AAMAD_TARGET_RUNTIME`:** `crewai` — source: `aamad.config.example.yml` (`runtime.target`); env var not set; no `aamad.config.yml` present
- **Adapter rules applied:** `.claude/rules/adapter-crewai.md` (YAML-first, sequential process, `memory=false`, `max_iter ≤ 12`, `max_retry_limit ≥ 2`, `Task.context` chaining, `Task.guardrail`, `allow_delegation=false` default per Sprint 1 tightening, `output_pydantic` binding, fixture-first tool policy)
- **Core rules applied:** `.claude/rules/aamad-core.md` (single responsibility, context-first, reproducibility, minimal viable architecture first, deterministic execution), `.claude/rules/adapter-registry.md`, `.claude/rules/development-workflow.md`
- **Template:** `.cursor/templates/sad-template.md` (v0.7.5 project family) — headings preserved; new subsections added under §1 ("Sprint 1 Scope (Vertical Slice)", "Deferred — Post-Sprint-1", "Sprint 1 Pinned Build Assumptions") and §2 ("Pydantic Output Contracts", "Orchestrator Pattern in `crew.py`", "Fixture-Tool Interface Contracts"). No template headings were removed.
- **Operator feedback applied (verbatim intent):** SAD read as product-complete MVP and Build risked swallowing weeks before a single grounded turn shipped. Five sharpenings applied:
    1. **Cut Sprint 1 to one vertical path** — exit criterion: anonymous/dev session → welcome qualify (stub CRM) → plan JSON in chat → one grounded support turn with citation (fixture KB) → escalate with context pack (stub ticket). Everything else (SSO polish, sponsor brief `human_input` gate, ambiguity calibration, live third-party APIs, 100 concurrent load, full F6 suite, per-agent memory) explicitly DEFERRED. ADR-013 added.
    2. **Name `output_pydantic` models** — added ADR-015; `CustomerProfile`, `OnboardingPlan`, `GroundedSupportAnswer`, `EscalationTicketPayload`, `AmbiguityReport`, `SponsorBrief` bound to their tasks in §2; JSON snippets kept as informative documentation; schema drift now fails Build.
    3. **Explicit sequential + branching in `crew.py`** — added ADR-016; §2 "Orchestrator Pattern" documents the per-turn phase router (welcome → plan|sponsor_brief|clarify|escalate; support loop; escalate on `support_failure_count>=2` OR explicit user request OR guardrail failure); `allow_delegation=false` tightened for Sprint 1 only (ADR-006 remains authoritative post-Sprint-1); `Task.context` propagates `CustomerProfile`.
    4. **Stub every external tool on day one** — added ADR-014; §2 "Fixture-Tool Interface Contracts" documents `crm_lookup`, `kb_vector_search`, `ticket_api`, and email hook fixture interfaces; env vars `{TOOL}_MODE=stub|live`; fixture KB requires 3–5 chunks with Path A (grounded hit) + Path B (no hit → clarify/escalate).
    5. **≤500ms first token / ≤3s p95 turn as a Week-3 spike gate** — pinned PIN-OQ-1 (LLM default = Anthropic Claude Sonnet-class, temperature ≤ 0.3) and PIN-OQ-2 (session cost ceiling = US$ 0.25/session working default) as Build assumptions so `max_iter`/`max_tokens` are not free variables. Latency remediation is bounded to model/prompt-size/context-window levers — adding agents or enabling memory is DISALLOWED as a Sprint 1 fix.
- **Preserved (per operator note — "gold-tier" content, NOT weakened):**
    - **ADR-011 Ambiguity handling contract** — unchanged in §1; `CustomerProfile.ambiguity_flags[]`, `ambiguity_report` schema, per-task guardrail bindings, `ambiguity-*.jsonl` observability hook, and F6 test suite (§9) all preserved. Sprint 1 exercises ONLY the escalation path; full calibration and telemetry DEFERRED.
    - **FE/BE SSE contract** — §4 SSE event envelope (`token`/`tool`/`cite`/`state`/`error`/`done`) unchanged; Sprint 1 vertical slice uses it as-is.
    - **§3 Frontend**, **§8 Security**, **§10 Launch** — unchanged in substance; only the Implementation Guidance was updated to reflect Sprint 1 vs. deferred scope.
- **Traceability updated:** added rows for Sprint 1 sharpening (ADR-013), fixture-first tools (ADR-014), Pydantic outputs (ADR-015), orchestrator pattern (ADR-016), and Week-3 spike gate.
- **Open Questions added:** items 18–21 (session-cost ceiling working value confirmation, "Talk to a human" intent-match policy, phase-router CI test acceptance, fixture KB seed content ownership).
- **Deferred without weakening:** OAuth SSO polish, ADR-008 human-input sponsor-brief gate, ADR-011 signal calibration + telemetry + full guardrail suite, live third-party APIs, 100 concurrent session load, container hardening, full pre-Deliver security assessment, per-agent memory. All remain SPECIFIED in this SAD; Build personas MUST NOT pull them into Sprint 1.
- **Artifact path:** `project-context/1.define/sad.md`
- **Write mode:** surgical edits via Edit tool (temp-write-then-atomic-replace semantics preserved for Windows/PowerShell)
- **Prompt Trace:** omitted at Define stage — deterministic template-driven scope-tightening off operator feedback; no material invention beyond feedback and prior SAD content; low-risk Define artifact. Prompt Trace will be captured for production-facing Build/Deliver artifacts per `aamad-core` policy.
- **SFS status:** no per-feature SFS files exist under `project-context/1.define/sfs/`; task specs live inline in this SAD §2. This audit action does not create new SFS files — the operator's feedback is applied inline to the SAD Crew Spec / Task definitions per the operator's instruction ("apply the five sharpenings above as edits to the existing SAD (and SFS where task specs live)"). Post-Sprint-1, if per-feature SFS files are requested, they will be authored under `project-context/1.define/sfs/<feature-id>.md`.
- **Next artifact:** `project-context/2.build/setup.md` by `@project.mgr` — MUST honor Sprint 1 vertical slice scope (§1) and scaffold `backend/tools/fixtures/`, `backend/tools/adapters/`, `backend/tools/factory.py`, `.env.example` `{TOOL}_MODE` vars per ADR-014. `@backend.eng` MUST bind `output_pydantic=<Model>` per ADR-015 and implement the phase router per ADR-016. `@qa.eng` MUST include the Week-3 spike gate measurement (≤500ms first-token / ≤3s p95 turn) as a Sprint 1 deliverable.
